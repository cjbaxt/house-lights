...
export default function WatchMenu({ current, onSelect, onClose }: Props) {
  ...
  return (
    <div
      ref={ref}
      className="absolute right-0 top-full mt-1 z-50 bg-white border border-neutral-100 rounded-xl shadow-sm py-1 w-44"
    >
      {OPTIONS.map(({ value, label }) => (
        <button
          ...
          className={`w-full text-left px-3 py-2 text-xs transition-colors hover:bg-neutral-50 ${
            current === value ? "text-neutral-900 font-medium" : "text-neutral-500"
          } ${value === null ? "text-neutral-300 border-t border-neutral-100 mt-1 pt-2" : ""}`}
        >
          {label}
        </button>
      ))}
