...
function ProgrammeCard({ show, allDates, location, watchMap, onWatchChange }: {
  ...
  return (
    <div className="bg-white border border-neutral-100 rounded-xl hover:border-neutral-300 transition-colors group">
            ...
            <div className="flex flex-wrap gap-1.5">
              {items.map(({ id, name }) => (
                <button key={id} onClick={() => toggleVenue(id)}
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${activeVenues.has(id) ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-200 text-neutral-500 hover:border-neutral-400"}`}
                >
                  {name}
                </button>
              ))}
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          ...
          className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-lg border transition-colors ${isFilterOpen ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-200 text-neutral-500 hover:border-neutral-400"}`}
        >
        ...
        <div className="flex items-center border border-neutral-200 rounded-lg overflow-hidden">
          {([
            ...
            <button key={key} onClick={() => setDisplayView(key)}
              className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 transition-colors ${displayView === key ? "bg-neutral-900 text-white" : "text-neutral-400 hover:bg-neutral-50"}`}
            >
              {icon}
              {label}
            </button>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-8 pr-8 py-2 text-sm border border-neutral-200 rounded-lg bg-white placeholder-neutral-400 text-neutral-800 focus:outline-none focus:border-neutral-400 transition-colors"
        />
        {searchQuery && (
          <button
      ...
      {/* Filter panel */}
      {isFilterOpen && (
        <div className="border border-neutral-200 rounded-xl p-4 mb-5 flex flex-col gap-5">
            ...
            <div className="flex items-center border border-neutral-200 rounded-lg overflow-hidden w-fit">
              {(["today", "week", "month", "all"] as Timeframe[]).map((t) => (
                <button key={t} onClick={() => setTimeframe(t)}
                  className={`text-xs px-3 py-1.5 transition-colors ${timeframe === t ? "bg-neutral-900 text-white" : "text-neutral-500 hover:bg-neutral-50"}`}
                >
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <button
                ...
                className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border transition-colors ${activeTypes.size === 0 ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-200 text-neutral-500 hover:border-neutral-400"}`}
              >
                All
              </button>
              {presentTypes.map((type) => (
                <button key={type} onClick={() => toggleType(type)}
                  className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border transition-colors ${activeTypes.has(type) ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-200 text-neutral-500 hover:border-neutral-400"}`}
                >
